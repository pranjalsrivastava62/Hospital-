...
void WelcomeScreen(void); //WelcomeScreen function declaration --->47
void Title(void);         //Title function declaration --->58
void MainMenu(void);      //MainMenu function declaration ---->65
void LoginScreen(void);   //LoginScreen function declaration ---->116
void Add_rec(void);       //Add_rec function declaration --->152
void func_list();         //function to list the patient details --->380
void Search_rec(void);    //Search_rec function declaration ---->434
void Edit_rec(void);      //Edit_rec function declaration
void Dlt_rec(void);       //Dlt_rec function declaration 
void ex_it(void);         //exit function declaration ---> 108
void gotoxy(short x, short y)
{
    COORD pos = {x, y}; //sets co-ordinates in (x,y). Here coord is structure with members  x and y
...
void WelcomeScreen(void) //function for welcome screen
{
    printf("\n\n\n\n\n\n\n\t\t\t\t#########################################");
    printf("\n\t\t\t\t#\t\tWELCOME TO\t\t#");
    printf("\n\t\t\t\t#   DELHI HOSPITAL MANAGEMENT SYSTEM    #");
    printf("\n\t\t\t\t#########################################");
...
void LoginScreen(void) //function for login screen
{
    ...
    char Username[15];  
    char Password[15];  
    char original_Username[25]="DTU";
    char original_Password[15] = "2K20";
    do
    {
    ...
    } while (e <= 2);//do while to run atleast once
    if (e > 2)
    {
...
A:
    ...
    else
    {
        for (b = 0; b < strlen(p.First_Name); b++)
        {
            ...
            {
                valid = 0;
                break;//we use break here because we are confirmed that string is containing an invalid character and we dont want to check it further
            }
        }
        if (!valid)
        {
            printf("\n\t\t First name contain Invalid characters :(....  Enter again :)");
            goto A;
        }
    }
/* ************************** Last name **************************** */
B:
    ...
    do
    {
        ...
        if (!ok)
        {
            printf("\n\t\t Invalid character :(  Enter either F or M :)");
        }
    } while (!ok);//if ok =0(invalid character) loop iterate again
    /* **********************Age ********************** */ 
    printf("\n\t\t\tAge:");
    scanf(" %i", &p.age);
    /* ******************** Address *********************** */ 
...
F:
    ...
    {
        printf("\n\t Invalid :( \t The max range for name is 30 and min range is 3 :)");
        goto F;
    }
    else
    {
    ...
    }                                     //\n is moving the cursor to next line for printing data there
...
void func_list()
{
    ...
    Title();
    FILE *ek;
    ek = fopen("Record2.dat", "r");//read mode
...
printf("================================================================================================================="); 
		row=17; //moves the cursor down after each run of loop
while(fscanf(ek,"%s %s %c %i %s %s %s %s %s\n", p.First_Name, p.Last_Name, &p.Gender, &p.age, p.Address, p.Contact_no, p.Email, p.Problem, p.Doctor)!=EOF)//EOF is 'end of file'  
		{                                 
        gotoxy(1, row);
...
void Search_rec(void)
{
    ...
    fflush(stdin);//to flush the output buffer of name which might appear, and get printed in subsequent printf statement 
    while (fscanf(ek, "%s %s %c %i %s %s %s %s %s\n", p.First_Name, p.Last_Name, &p.Gender, &p.age, p.Address, p.Contact_no, p.Email, p.Problem, p.Doctor) != EOF)
    {                                          //\n move the control to next line of file to scan data from it.
...
void Edit_rec(void)
{
    ...
    scanf(" %s", name);
    fflush(stdin); //to flush the input buffer of name which might appear, and get printed in subsequent printf statement
    name[0] = toupper(name[0]);
    gotoxy(12, 15);
    if (ft == NULL)
    {
    ...
    while (fscanf(ek, "%s %s %c %i %s %s %s %s %s\n", p.First_Name, p.Last_Name, &p.Gender, &p.age, p.Address, p.Contact_no, p.Email, p.Problem, p.Doctor) != EOF)
    {
        ...
        F1:
            ...
            printf("\n\t\t\tPress U character to Update record : ");
            ch = getche();
            if (ch == 'u' || ch == 'U')
            {
